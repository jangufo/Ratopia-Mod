...
生成的 `ratopia-ci-deps.zip` 会扫描仓库内所有非测试项目与 `Directory.Build.props`，收集 `BepInEx/core` 和 `Ratopia_Data/Managed` 下所有被引用的 DLL。当前 CI 使用私有依赖仓库 `jangufo/ratopia-ci-deps` 的 `deps-v1` Release；不要提交或公开发布这些游戏文件。
...
全量构建与发布由 `.github/workflows/build-and-release-mods.yml` 负责。它会在每次触发时：

1. 下载私有依赖包；
2. 编译仓库内全部非测试 mod；
3. 为每个 mod 生成 `<AssemblyName>-v<Version>.zip`；
4. 额外生成全量集合包 `Ratopia-Mod-202609072051.zip`；
5. 在正式 tag（例如 `yunqing-v3.0.0`）触发时，创建 GitHub Release 并同时上传：
   - 当前 mod 的单独包
   - 全量集合包 `Ratopia-Mod-202609072051.zip`
   - GitHub 自动生成的两个 Source code 压缩包

Release 正文会优先使用 `docs/release-notes/<tag>.md`，不存在时回退到 `docs/release-notes/DEFAULT.md`。请在对应文件里写清楚每个版本的具体更新内容。
